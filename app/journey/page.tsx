      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative">
          ...
          {timelineData.map((item, index) => (
            <motion.div
              key={index}
              className="relative mb-32 flex flex-col md:flex-row items-center gap-12" // Dùng gap để tạo khoảng cách cố định
            >
              {/* 1. TRỤC GIỮA (Năm) - Chỉ hiện trên Desktop */}
              <div className="absolute left-1/2 -translate-x-1/2 hidden md:flex flex-col items-center z-20">
                <div className={`w-12 h-12 rounded-2xl ${item.color} flex items-center justify-center text-white shadow-lg mb-2`}>
                  <Calendar size={20} />
                </div>
                <div className="bg-white px-3 py-1 rounded-full border border-stone-100 shadow-sm">
                  <span className="font-black text-stone-800 text-sm">{item.year}</span>
                </div>
              </div>

              {/* 2. BÊN TRÁI: NỘI DUNG (Dùng cho index chẵn) */}
        ...
        {/* 3. BÊN PHẢI: HÌNH ẢNH (Dùng cho index chẵn) */}
        <div className={`w-full md:w-1/2 overflow-hidden ${index % 2 === 0 ? 'md:order-2' : 'md:order-1'}`}>
  <div className="relative group cursor-grab active:cursor-grabbing">
    <motion.div
      className="flex gap-6 flex-nowrap" // Tăng gap lên 6 để thoáng hơn
      style={{ width: "max-content" }}
      animate={{ x: ["0%", "-50%"] }}
      transition={{
        duration: 25, // Tăng nhẹ thời gian để trượt mượt hơn vì ảnh rộng hơn
        ease: "linear",
        repeat: Infinity,
      }}
    >
      {[...item.images, ...item.images].map((img, i) => (
        <div
          key={i}
          // Tăng chiều rộng từ 250px lên 350px-400px
          // Đổi aspect-[4/5] thành aspect-video (16:9) hoặc aspect-[16/10] để ảnh rộng hơn
          className="flex-none w-[300px] md:w-[400px] aspect-[16/10] relative rounded-[1.5rem] overflow-hidden shadow-lg border border-stone-200/50"
        >
          <Image
            src={img}
            alt="Activity"
            ...
            // QUAN TRỌNG: Tăng chất lượng ảnh bằng cách chỉnh sizes lớn hơn thực tế một chút
            // hoặc thêm thuộc tính quality={90}
            sizes="(max-width: 768px) 300px, 450px"
            quality={90} 
            priority={i < 2} // Ưu tiên load rõ nét cho những ảnh đầu tiên
          />
        </div>
      ))}
    </motion.div>
    ...
    {/* Lớp phủ mờ */}
    <div className="absolute inset-y-0 left-0 w-16 bg-gradient-to-r from-[#fcfcfb] to-transparent z-10" />
    <div className="absolute inset-y-0 right-0 w-16 bg-gradient-to-l from-[#fcfcfb] to-transparent z-10" />
  </div>
</div>
      </motion.div>
          ))}
